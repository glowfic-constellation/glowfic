...
class PostsController < WritableController
  ...
  def unread
    ...
    @posts = @posts.select { |p| p.visible_to?(current_user, !!params[:show_blocked]) }
  ...
  def mark
    ...
    posts = posts.select { |post| post.visible_to?(current_user, true) }
  ...
  def editor_setup
    super
    @permitted_authors = (User.ordered.where.not(id: current_user.blocked_interaction_users) - (@post.try(:joined_authors) || []))
