class Api::V1::SubcontinuitiesController < Api::ApiController
  ...
  def reorder
    ...
    sections = Subcontinuity.where(id: section_ids)
    ...
    continuities = Continuity.where(id: sections.select(:continuity_id).distinct.pluck(:continuity_id))
    ...
    Subcontinuity.transaction do
      ...
      other_sections = Subcontinuity.where(continuity_id: continuity.id).where.not(id: section_ids).ordered
    ...
    render json: {section_ids: Subcontinuity.where(continuity_id: continuity.id).ordered.pluck(:id)}
  end
end
