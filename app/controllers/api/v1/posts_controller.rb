class Api::V1::PostsController < Api::ApiController
  ...
  def reorder
    ...
    unless post_section_ids == [section_id] &&
           (section_id.nil? || BoardSection.where(id: section_id, board: board).exists?)
    ...
    Post.transaction do
      ...
      other_posts = Post.where(board: board, section_id: section_id).where.not(id: post_ids).ordered_in_section
    ...
    posts = Post.where(board: board, section_id: section_id)
