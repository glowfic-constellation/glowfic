class Api::V1::BoardsController < Api::ApiController
  ...
  def show
    unless (continuity = Board.find_by_id(params[:id]))
      error = { message: "Continuity could not be found." }
      render json: { errors: [error] }, status: :not_found and return
    end

    render json: continuity.as_json(include: [:board_sections])
  end
  ...
  def posts
    ...
    queryset = Post.privacy_public.where(board: continuity).with_reply_count.select('posts.*')
