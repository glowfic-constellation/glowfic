...
module Authentication::Api
  ...
  included do
    ...
    # Only access tokens authenticate API requests. Scoping to Oauth2Token
    # keeps authorization codes (Oauth2Verifier, which shares the oauth_tokens
    # table and stores the code in `token`) from being usable as bearer tokens.
    def oauth_token_user
      token_value = request.headers['Authorization'].to_s.split(' ').last
      return unless token_value.present?
      Oauth2Token.active.find_by(token: token_value)&.user
    end
