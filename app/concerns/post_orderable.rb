module PostOrderable
  ...
  included do
    ...
    def reorder_others
      ...
      others = self.class.where(other_where).order('section_order asc').to_a
      others += BoardSection.where(board_id: board_id_was).order('section_order asc').to_a
      return unless others.present?
      others.sort_by!{ |i| i.section_order }
