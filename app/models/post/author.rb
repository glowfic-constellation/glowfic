class Post::Author < ApplicationRecord
  ...
  def self.clear_cache_for(authors)
    blocked_ids = Block.where(blocking_user: authors, hide_me: [Block::POSTS, Block::ALL]).pluck(:blocked_user_id)
    blocked_ids.each { |blocked| Rails.cache.delete(Block.cache_string_for(blocked, 'blocked')) }
    hiding_ids = Block.where(blocked_user: authors, hide_them: [Block::POSTS, Block::ALL]).pluck(:blocking_user_id)
