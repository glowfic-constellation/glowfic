class CharacterTag < ApplicationRecord
  ...
  def add_galleries_to_character
    ...
    galleries = gallery_group.galleries.where(user_id: character.user_id).where.not(id: joined_galleries.select(:id))
  ...
  def remove_galleries_from_character
    ...
    joined_group_galleries = character.gallery_groups.joins(:galleries).where(galleries: {user_id: character.user_id}).select(:gallery_id)
