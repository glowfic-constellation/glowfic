...
class CharacterTag < ApplicationRecord
  ...
  def single_group_for_character
    return unless tag.is_a?(CharacterGroup)
    return unless character.character_tags.joins(:tag).where(tags: { type: 'CharacterGroup' }).where.not(tag: character_group).exists?
