...
group_cross = OldCharacterGroup.ids.index_with(nil)

OldCharacterGroup.find_each do |group|
...
templates.each do |template|
  ...
  TemplateTag.create!(template: template, tag_id: group_cross[group_id], section_order: 0)
end

untemplated_characters.each do |character|
  CharacterTag.create(character: character, tag_id: group_cross[character.character_group_id], section_order: 0)
end
