RSpec.describe RepliesController, 'POST restore' do
  ...
  it "handles first reply deletion" do
    ...
    deleted_reply.destroy!
    Timecop.freeze(rpost.reload.tagged_at + 1.day) { reply }
    post_attributes = Post.find_by(id: rpost.id).attributes
  ...
  it "handles first reply deletion without written" do
    rpost.written.delete
    expect(rpost.reload.written).to be_nil

    replies = create_list(:reply, 2, post: rpost, user: user)
    deleted_reply = replies.first
    deleted_reply.destroy!
    Timecop.freeze(rpost.reload.tagged_at + 1.day) { reply }
    post_attributes = rpost.reload.attributes
  ...
  it "handles last reply deletion" do
    ...
    deleted_reply = Timecop.freeze(rpost.reload.tagged_at + 1.day) { reply }
    deleted_reply.destroy!
    post_attributes = Post.find_by(id: rpost.id).attributes
    ...
    expect(reloaded_post.last_user).to eq(user)
  ...
  it "correctly restores a previously restored reply" do
    reply = Timecop.freeze(2.days.ago) { create(:reply, user: user, content: 'not yet restored') }
  ...
  it "correctly restores the reply's created_at date" do
    deleted_reply = Timecop.freeze(1.day.ago) { reply }
    old_created_at = deleted_reply.created_at
    deleted_reply.destroy!
    login_as(user)
    ...
    expect(flash[:success]).to eq("Reply restored.")
    reply = Reply.find(deleted_reply.id)
