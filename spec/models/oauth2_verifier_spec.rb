RSpec.describe Oauth2Verifier do
  ...
  let(:client_application) do
    ClientApplication.create! user: user, name: "Client Application name", url: "http://localhost/",
      callback_url: "http://localhost:3000/callback"
  end
  let(:verifier) { Oauth2Verifier.create! client_application: client_application, user: user, scope: "bbbb aaaa" }
