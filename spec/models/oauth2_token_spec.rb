RSpec.describe Oauth2Token do
  ...
  let(:client_application) { ClientApplication.create! user: user, name: "Client application", url: "http://localhost", callback_url: "http://localhost:3000" }
  let(:token) { Oauth2Token.create! client_application: client_application, user: user }
