module MultiFetchFragments
  ...
  def render_collection_with_multi_fetch_cache
    ...
    if cache_collection?
      ...
      @collection.each do |item|
        expanded_key = key_for_item(item)
      ...
      # allow users to specify keys they don't want fetch from cache
      cache_keys = mutable_keys
      cache_keys = mutable_keys - [key_for_item(@options[:uncached])] if @options[:uncached].present?

      result_hash = {}
      result_hash = Rails.cache.read_multi(*cache_keys) unless cache_keys.empty?
  ...
  def key_for_item(item)
    ...
    fragment_cache_key(key_with_optional_digest)
  end
end
